...
\chapter{Auswertung der Daten}
	...
	Nach den beiden beschriebenen Eichprozessen, kann die eigentliche Langzeitmessung beginnen. Start der Messung war am 31.05.2024 um 13:45, beednet wurde die Messung 10 Tage später am 10.05.2024 um 16:08. Daraus folgt eine Messdauer von:
	\begin{equation}
		T_{ges} =  \SI{10}{\day} \; \SI{2}{\hour} \; \SI{23}{\minute} \equiv  \SI{872580}{\second}
	\end{equation}
	Dabei sind insgesamt $1.7$ Mio. Myonen durch den Szintillator geflogen, aber nur etwa $3600$ von diesen, sind als im Szintillator feststeckendes Ereignis aufgezeichnet wurden.
	\begin{itemize}
		\item $N_{ges} = 1.7E6$
		\item $N_{mess} = 3673$
	\end{itemize}
	Wie schon mal beschreiben hat der ADC 8064 Kanäle, diese feine Auflösung, führt dazu, dass in den hinteren Kanälen keine oder sehr wenige Einträge sind, da die Messergebnisse exponentiell abfallen. Die Lösung ist das sogenannte \textit{rebinning}, hierfür werden mehrere Kanäle zu einem neuen Zusammengefasst, ihrer Einträge dafür addiert. Es wurde in dieser Auswertung der Faktor 128 gewählt, dadurch wird auf 63 Bins reduziert, keiner der neuen Bins hat den Eintrag null.\\
	...
	\begin{equation}
		\mu = \frac{\text{Ereignisse}}{\text{Messzeit}} = \frac{N_{ges}}{T_{ges}} = \frac{1.7E6}{\SI{872580}{\second}} = \SI{1.948}{\per \second}
	\end{equation}
	Damit bestimmt sich die Wahrscheinlichkeit zu:
	\begin{equation}
			P_{\mu} (\Delta t, n  \geq 1) = 1- e^{-\mu \Delta t} = 2.183E-7
	\end{equation}
	...
	\end{figure}
	Wie im Theorieteil beschrieben folgen die Messdaten einer Exponentialfunktion, in Abbildung \ref{img:langzeitMess}, ist zu erkennen, dass solch eine Funktion sinnvoll erst ab einer Zeitdifferenz von etwa \SI{2400}{\nano \s} gefittet werden kann. Dadurch verschlechtert sich die Statistik, von den ursprünglichen 3673 Einträgen, kann nur an etwa 1200 gefittet werden. Es wird folgende Funktion mit einem gewichteten Fit verwendet:
	\begin{equation}
	...
	\end{figure}
	Das Ergebnis des Fits ist in Abbildung \ref{img:FitlangzeitMess}, sowohl mit linearer Achse, als auch logarithmisch aufgetragen. Die Konstante $\lambda$ hat den Wert \SI{5.8494(0.2542)E-4}{\nano \s}. Die mittlere Lebensdauer lässt sich einfach durch ihr reziprokes bestimmen:
	\begin{equation}
		\highlight{\tau} = \frac{1}{\lambda} = \highlight{\SI{1709.579(74.296)}{\nano \s}}
	\end{equation}
\chapter{Fazit}
	\fehlt
